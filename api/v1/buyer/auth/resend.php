...
$stmt = $pdo->prepare('SELECT id, name, email_verified_at FROM buyers WHERE email = ? AND deleted_at IS NULL');
$stmt->execute([$email]);
$buyer = $stmt->fetch();

if (!$buyer) api_json(['error' => 'not_found'], 404);

// Nothing to send. The app sends this buyer to sign in rather than leaving
...
$pdo->prepare('UPDATE buyers SET verify_token = ?, verify_code_expires = ? WHERE id = ?')
    ->execute([$code, $expires, $buyer['id']]);
